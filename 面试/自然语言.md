...
模型向量化:bge微调
样本：问题->百川生成样本增强，作为正样本 相似度>0.7     问题向量化后取相似度0.3-0.6作为负样本，尽可能大  得到7k三元组
模型： 预训练：retroMae 微调：对比学习
评估：MRR@k Recall@k  (Mean Reciprocal Rank平均倒数排序) 测试集构建：一条q，根据相似度召回20条相似正例，人工质检 得到6百q-qs样本对
对比：
