...
模型向量化:m3e微调( Moka Massive Mixed Embedding Model)
样本：问题->样本增强（删减/相似替换）：正      问题->随机采样：负  得到9k三元组
模型： 双塔bert，loss:**TripletInBatchNegCoSentLos**s 计算句子与正负例的相似度差值。希望正例相似度减负例相似度的差值越大。
评估：
对比：
